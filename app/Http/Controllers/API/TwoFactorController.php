...
class TwoFactorController extends Controller
{
  ...
  // Внедряем зависимость TwoFactorService, для использоваие методов  в контроллере конструктоом
  ...
  public function requestNewCode(Request $request)
  {
    ...
    if (!$userToken) {
      return response()->json(['message' => 'Tak sebe token, need another'], 401);
    }
    ...
    // Если запросы превышают maxRequests за указанный период, блокируется запрос
      ...
      if ($user->last_request_time) {
        ...
        if ($secondsSinceLastRequest < $delaySeconds) {
          ...
          return response()->json([
            'message' => 'Too many requests pleasee try again ' . ceil($waitTime) . ' second.'
          ], 429);
        } else {
    ...
    return response()->json(['message' => '2FA code  sent again']);
  }
  ...
  public function toggleTwoFactor(Request $request)
  {
    ...
    if (!Hash::check($currentPassword, $user->password)) {
      return response()->json(['message' => 'Fail password'], 400);
    }
    ...
    return response()->json([
      'message' => $user->is_two_fa_enabled ? '2FA ON' : '2FA OFF',
    ]);
  }
}
